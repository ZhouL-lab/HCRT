...
class Lits_DataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pre = self.normalization(self.load(os.path.join(self.root, 'image_p0', '{}_P0.nii.gz'.format(file)))).astype(
            np.float32)
        pos = self.normalization(self.load(os.path.join(self.root, 'image_p1', '{}_P1.nii.gz'.format(file)))).astype(
            np.float32)
        sub = pos - pre
        gt = self.load(os.path.join(self.root, 'label', '{}_GT.nii.gz'.format(file))).astype(np.float32)
        # masks = np.zeros_like(pre)
        # n=index
        # print('mask creat finish{},{},{}'.format(n,file,boxdata[n][0]))
        # print('hjsiadha',index,np.count_nonzero(gt))
        pre_patch, pos_patch, sub_patch, gt_patch = [], [], [], []
        for i in range(3):
            if i == 1:
                pre_patch1, pos_patch1, sub_patch1, gt_patch1 = self.random_crop_3d_contain(pre, pos, sub, gt,
                                                                                            self.size)

            else:
                pre_patch1, pos_patch1, sub_patch1, gt_patch1 = self.random_crop_3d_partial(pre, pos, sub, gt,
                                                                                            self.size)
            # print(index,i,np.count_nonzero(gt_patch1))
            pre_patch.append(pre_patch1), pos_patch.append(pos_patch1), sub_patch.append(sub_patch1), gt_patch.append(
                gt_patch1)
    ...
    def random_crop_3d_contain(self, pre, pos, sub, gt, crop_size):
        ...
        random_x_min, random_x_max = max(cor_box[0, 1] - crop_size[0], 0), min(cor_box[0, 0],
                                                                               pre.shape[0] - crop_size[0])
        random_y_min, random_y_max = max(cor_box[1, 1] - crop_size[1], 0), min(cor_box[1, 0],
                                                                               pre.shape[1] - crop_size[1])
        random_z_min, random_z_max = max(cor_box[2, 1] - crop_size[2], 0), min(cor_box[2, 0],
        ...
        gt_patch = gt[x_random:x_random + crop_size[0], y_random:y_random + crop_size[1],
                   z_random:z_random + crop_size[2]]
        # masks_patch = masks[x_random:x_random + crop_size[0], y_random:y_random + crop_size[1], z_random:z_random + crop_size[2]]
