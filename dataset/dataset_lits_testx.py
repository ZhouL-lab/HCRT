...
class Img_DataSetv2(Dataset):
    def __init__(self, pre, pos, sub, cut_param):
        self.pre = pre
        self.pos = pos
        self.sub = sub
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(pre), torch.from_numpy(pos), torch.from_numpy(sub)
...
def Test_Datasets(dataset_path, size, test_folder=1):
    f = open(os.path.join(dataset_path, 'data_folder', 'testrj' + str(test_folder) + '.txt'))
...
def readtxt():
    txt_path = '/media/usst/data/resunet/data/data_folder.txt'  # r对路径进行转义，windows需要
    t = []
    p = []
...
def Test_all_Datasets(dataset_path, size, test_folder=1):
    ...
    for file in data_list:
        ...
        # pre = normalization(load(os.path.join(dataset_path,file,'P0.nii.gz'))).astype(np.float32)
        # pos = normalization(load(os.path.join(dataset_path,file,'P1.nii.gz'))).astype(np.float32)
        pre = normalization(load(os.path.join(dataset_path, 'image_p0', '{}_P0.nii.gz'.format(file)))).astype(
            np.float32)
        pos = normalization(load(os.path.join(dataset_path, 'image_p1', '{}_P1.nii.gz'.format(file)))).astype(
            np.float32)
        sub = pos - pre
        # print(sub.shape)
        # print(sub.shape)
        gt = load(os.path.join(dataset_path, 'label', '{}_GT.nii.gz'.format(file))).astype(np.int16)
        # dataset_path1 = '/media/dell/D/breastTumorSeg/TMI/breastmask'
        dataset_path1 = '/media/dell/D/breastTumorSeg/YN/breastmask'
        breast_mask = load(os.path.join(dataset_path1, file + '.nii.gz')).astype(np.int16)
        original_shape = pre.shape
        new_shape = cal_newshape(pre, size)
